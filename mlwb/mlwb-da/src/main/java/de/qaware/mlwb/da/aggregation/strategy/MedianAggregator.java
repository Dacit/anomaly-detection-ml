 ...
 * @author Fabian Huch
 */
public class MedianAggregator implements AggregateStrategy {
    @Override
    public ValuePoint aggregate(Collection<ValuePoint> valuePoints, Granularity granularity) {
        ...
        List<Double> groupValues = new LinkedList<>();

        for (ValuePoint v : valuePoints) {
            groupValues.add(v.getValue());
        }
        Collections.sort(groupValues);
        return new ValuePoint(groupValues.get(groupValues.size() / 2), date);
    }
}
